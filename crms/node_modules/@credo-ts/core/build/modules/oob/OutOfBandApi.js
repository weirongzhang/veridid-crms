...
let OutOfBandApi = class OutOfBandApi {
    ...
    async receiveImplicitInvitation(config) {
        const handshakeProtocols = this.getSupportedHandshakeProtocols(config.handshakeProtocols ?? [connections_1.HandshakeProtocol.DidExchange]).map((p) => p.parsedProtocolUri.protocolUri);
    ...
    getSupportedHandshakeProtocols(limitToHandshakeProtocols) {
        const allHandshakeProtocols = limitToHandshakeProtocols ?? Object.values(connections_1.HandshakeProtocol);
    ...
    ensureParentThreadId(outOfBandRecord, plaintextMessage) {
        ...
        if (plaintextMessage['~thread']?.pthid &&
