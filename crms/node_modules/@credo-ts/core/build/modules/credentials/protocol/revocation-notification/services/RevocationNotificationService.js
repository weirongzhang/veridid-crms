...
let RevocationNotificationService = class RevocationNotificationService {
    ...
    async processRevocationNotification(agentContext, anonCredsRevocationRegistryId, anonCredsCredentialRevocationId, connection, comment) {
        ...
        if (!credentialExchangeRecord && revocationIdentifier_1.unqualifiedRevocationRegistryIdRegex.test(anonCredsRevocationRegistryId)) {
            ...
            this.logger.trace('Credential Exchange Record not found. Getting W3C credential record by query for revocation notification:', w3cCredentialQuery);
            ...
            credentialExchangeRecord =
                (await this.credentialRepository.findByQuery(agentContext, {
                    $or: [{ state: models_1.CredentialState.CredentialReceived }, { state: models_1.CredentialState.Done }],
        ...
        if (!credentialExchangeRecord) {
            throw new error_1.CredoError(`No associated Credential Exchange record found for revocation id ${anonCredsCredentialRevocationId}`);
        }
    ...
    async v1ProcessRevocationNotification(messageContext) {
        ...
        try {
            ...
            if (!threadIdGroups) {
                throw new error_1.CredoError(`Incorrect revocation notification threadId format: \n${threadId}\ndoes not match\n"indy::<revocation_registry_id>::<credential_revocation_id>"`);
            }
    ...
    async v2ProcessRevocationNotification(messageContext) {
        ...
        if (![revocationIdentifier_1.v2IndyRevocationFormat, revocationIdentifier_1.v2AnonCredsRevocationFormat].includes(messageContext.message.revocationFormat)) {
            throw new error_1.CredoError(`Unknown revocation format: ${messageContext.message.revocationFormat}. Supported formats are indy-anoncreds and anoncreds`);
        }
        try {
            ...
            if (!credentialIdGroups) {
                throw new error_1.CredoError(`Incorrect revocation notification credentialId format: \n${credentialId}\ndoes not match\n"<revocation_registry_id>::<credential_revocation_id>"`);
            }
