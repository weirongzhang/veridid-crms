...
let ConnectionService = class ConnectionService {
    ...
    async processAck(messageContext) {
        ...
        if (!connection) {
            throw new error_1.CredoError(`Unable to process connection ack: connection for recipient key ${recipientKey?.fingerprint} not found`);
        }
    ...
    async assertConnectionOrOutOfBandExchange(messageContext, { lastSentMessage, lastReceivedMessage, expectedConnectionId, } = {}) {
        ...
        if (expectedConnectionId && connection?.id !== expectedConnectionId) {
            throw new error_1.CredoError(`Expected incoming message to be from connection ${expectedConnectionId} but connection is ${connection?.id}.`);
        }
    ...
    async matchIncomingMessageToRequestMessageInOutOfBandExchange(messageContext, { expectedConnectionId }) {
        if (expectedConnectionId && messageContext.connection?.id !== expectedConnectionId) {
            throw new error_1.CredoError(`Expecting incoming message to have connection ${expectedConnectionId}, but incoming connection is ${messageContext.connection?.id ?? 'undefined'}`);
        }
        const outOfBandRepository = messageContext.agentContext.dependencyManager.resolve(repository_2.OutOfBandRepository);
    ...
    createDidDocFromOutOfBandDidCommServices(services) {
        ...
        const auth = new models_1.ReferencedAuthentication(publicKey, models_1.authenticationTypes.Ed25519VerificationKey2018);
        // IndyAgentService is old service type
        const service = services.map((service, index) => new dids_1.IndyAgentService({
            id: `${did}#IndyAgentService-${index + 1}`,
            serviceEndpoint: service.serviceEndpoint,
            recipientKeys: [recipientKey.publicKeyBase58],
            routingKeys: service.routingKeys?.map(helpers_1.didKeyToVerkey),
            priority: index,
        }));
        return new models_1.DidDoc({
            id: did,
