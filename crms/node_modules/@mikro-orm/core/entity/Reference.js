...
class Reference {
    ...
    async load(options = {}) {
        ...
        options = { ...options, filters: QueryHelper_1.QueryHelper.mergePropertyFilters(this.property?.filters, options.filters) };
        ...
        if (!this.isInitialized() || options.refresh) {
            if (options.dataloader ?? [enums_1.DataloaderType.ALL, enums_1.DataloaderType.REFERENCE].includes(DataloaderUtils_1.DataloaderUtils.getDataloaderType(wrapped.__em.config.get('dataloader')))) {
