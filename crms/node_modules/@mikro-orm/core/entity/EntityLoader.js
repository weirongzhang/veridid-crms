...
class EntityLoader {
    ...
    async findChildren(entities, prop, populate, options, ref) {
        ...
        if ([enums_1.ReferenceKind.ONE_TO_ONE, enums_1.ReferenceKind.MANY_TO_ONE].includes(prop.kind) && items.length !== children.length) {
            ...
            const isInverseOneToOne = prop.kind === enums_1.ReferenceKind.ONE_TO_ONE && !prop.owner;
            for (const entity of entities) {
                const ref = entity[prop.name];
                if (ref == null) {
                    continue;
                }
                const refPk = (0, wrap_1.helper)(ref).getSerializedPrimaryKey();
                ...
                const childKey = isInverseOneToOne
                    ? (0, wrap_1.helper)(entity).getSerializedPrimaryKey()
                    : refPk;
