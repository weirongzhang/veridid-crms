...
class ChangeSetPersister {
    ...
    syncCompositeIdentifiers(changeSet) {
        ...
        for (let i = 0; i < pks.length; i++) {
            ...
            if (ident instanceof entity_1.EntityIdentifier && pks[i].kind === enums_1.ReferenceKind.SCALAR) {
    ...
    processProperty(changeSet, prop) {
        ...
        if (Array.isArray(value) && value.some(item => item instanceof entity_1.EntityIdentifier)) {
            changeSet.payload[prop.name] = value.map(item => item instanceof entity_1.EntityIdentifier ? item.getValue() : item);
            return;
        }
